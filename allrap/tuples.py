...
def diluted_nearby_tuple_aligned_coord_loss(poses, split, k, cpa_with_grad=False,z_scale=False):
    if split < 2:
        return 0
    B, K, THREE = poses.shape
    s = split
...
def outlier_loss(pred_pose_3d,threshold=1.0):
    ...
    x=pairwise_distances(x).topk(2,dim=2,largest=False)[0][:,:,1:2].sub(threshold).relu()
    return x.mean()
...
if os.path.exists('/scratch/benjamingraham/tmp_dir/'):
    vis = visdom.Visdom(env="q3", port=8097, server="100.97.72.139")  # devfair0301
else:
    vis=None
...
def render_pcl(xyz,rgb,radius=0.1,view='F',res=512):
    ...
    scene.add(pyrender.Mesh.from_trimesh(sm,                   material=material
                                        ),pose=torch.eye(4))
    if view=='F':
